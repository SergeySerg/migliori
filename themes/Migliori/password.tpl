		...
		<form action="{$request_uri|escape:'html':'UTF-8'}" method="post" class="std" id="form_forgotpassword">
			<fieldset>
				<div class="form-group">
					...
					<input class="form-control" placeholder="{l s='Email address'}" type="email" id="email" name="email" value="{if isset($smarty.post.email)}{$smarty.post.email|escape:'html':'UTF-8'|stripslashes}{/if}" />
				</div>
				<p class="submit">
					<button type="submit" class="btn btn-default button mg_btn">{l s='Retrieve Password'}</button>
				</p>
			</fieldset>
		</form>
	{/if}
	<a class="btn btn-default mg_btn" href="{$link->getPageLink('authentication')|escape:'html':'UTF-8'}" title="{l s='Back to Login'}" rel="nofollow">{l s='Back to Login'}</a>
</div>
{* <ul class="clearfix footer_links">
	<li>
		<a class="btn btn-default button mg_btn" href="{$link->getPageLink('authentication')|escape:'html':'UTF-8'}" title="{l s='Back to Login'}" rel="nofollow">{l s='Back to Login'}</a>
	</li>
</ul> *}
