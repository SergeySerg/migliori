...
class PcXmlFreeFeed extends ObjectModel
{
	...
	public function update($null_values = false, $autodate = true)
	{
		...
		try {
			...
			if (self::$db->getNumberError() == 1062) {
				throw new RuntimeException(sprintf(Tools::displayError('Zadaný název XML souboru (%s) už existuje.'), $this->filename));
			} else {
				throw $e;
			}
		}
	...
	static public function updateFiles(array $data) {
		$sql = '';
		foreach ($data as $file) {
			$sql .= "UPDATE `"._DB_PREFIX_.self::$definition['table']."` SET `filesize` = '".(int)$file['filesize']."', `created` = '".date('Y-m-d H:i:s', $file['lastmod'])."' WHERE `".self::$definition['primary']."` = ".(int)$file[self::$definition['primary']].";\n";
		}
