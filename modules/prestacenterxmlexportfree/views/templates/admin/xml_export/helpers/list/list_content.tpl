		...
		{foreach $fields_display AS $key => $params}
			...
			{block name="td_content"}
				...
				{elseif isset($params.activeVisu)}
					...
					alt="{if $tr.$key}{l s='Povoleno' mod='prestacenterxmlexportfree'}{else}{l s='Zakázáno' mod='prestacenterxmlexportfree'}{/if}" title="{if $tr.$key}{l s='Povoleno' mod='prestacenterxmlexportfree'}{else}{l s='Zakázáno' mod='prestacenterxmlexportfree'}{/if}" />
				{elseif isset($params.position)}
					{if $order_by == 'position' && $order_way != 'DESC'}
						<a href="{$tr.$key.position_url_down}" {if !($tr.$key.position != $positions[count($positions) - 1])}style="display: none;"{/if}>
							<img src="../img/admin/{if $order_way == 'ASC'}down{else}up{/if}.gif" alt="{l s='Dolů' mod='prestacenterxmlexportfree'}" title="{l s='Dolů' mod='prestacenterxmlexportfree'}" />
						</a>
						<a href="{$tr.$key.position_url_up}" {if !($tr.$key.position != $positions.0)}style="display: none;"{/if}>
							<img src="../img/admin/{if $order_way == 'ASC'}up{else}down{/if}.gif" alt="{l s='Nahoru' mod='prestacenterxmlexportfree'}" title="{l s='Nahoru' mod='prestacenterxmlexportfree'}" />
						</a>
					{else}
						{$tr.$key.position + 1}
					{/if}
...
{/foreach}
{else}
	<tr><td class="center" colspan="{count($fields_display) + 2}">{l s='Nebyly nalezeny žádné položky' mod='prestacenterxmlexportfree'}</td></tr>
{/if}
</tbody>
