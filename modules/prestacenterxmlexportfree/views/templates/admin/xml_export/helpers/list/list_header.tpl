			...
			<tr>
				<td style="vertical-align: bottom;">
					<span style="float: left;">
						...
						{/if}
						{l s='Stránka' mod='prestacenterxmlexportfree'} <b>{$page}</b> / {$total_pages}
						{if $page < $total_pages}
						...
						{/if}
						| {l s='Zobrazit výsledky:' mod='prestacenterxmlexportfree'}
						...
						/ {$list_total} {l s='x' mod='prestacenterxmlexportfree'}
					</span>
					<span style="float: right;">
						<input type="submit" name="submitReset{$table}" value="{l s='Resetovat' mod='prestacenterxmlexportfree'}" class="button" />
						<input type="submit" id="submitFilterButton{$table}" name="submitFilter" value="{l s='Filtr' mod='prestacenterxmlexportfree'}" class="button" />
					</span>
							...
							<th class="center">
								{if $has_bulk_actions || (isset($xmlexport.cbx.show) && $xmlexport.cbx.show)}
									...
									{if !isset($xmlexport.cbx.dependent) && !$xmlexport.cbx.dependent}
										onclick="checkDelBoxes(this.form, '{$table}Box[]', this.checked)"
									{/if} />
								{/if}
							</th>
							{foreach $fields_display AS $key => $params}
							...
							{if $shop_link_type}
								<th>
									{if $shop_link_type == 'shop'}
										{l s='Obchod' mod='prestacenterxmlexportfree'}
									{else}
										{l s='Skupina obchodů' mod='prestacenterxmlexportfree'}
									{/if}
									<br />&nbsp;
								</th>
							{/if}
							{if $has_actions}
								<th class="center">{l s='Akce' mod='prestacenterxmlexportfree'}{if !$simple_header}<br />&nbsp;{/if}</th>
							{/if}
						</tr>
 						{if !$simple_header}
									...
									{else}
										{if $params.type == 'bool'}
											<select onchange="$('#submitFilterButton{$table}').focus();$('#submitFilterButton{$table}').click();" name="{$table}Filter_{$key}">
												<option value="">--</option>
												<option value="1" {if $params.value == 1} selected="selected" {/if}>{l s='Ano'  mod='prestacenterxmlexportfree'}</option>
												<option value="0" {if $params.value == 0 && $params.value != ''} selected="selected" {/if}>{l s='Ne' mod='prestacenterxmlexportfree'}</option>
											</select>
										{elseif $params.type == 'date' || $params.type == 'datetime'}
											{l s='Od' mod='prestacenterxmlexportfree'} <input type="text" class="filter datepicker" id="{$params.id_date}_0" name="{$params.name_date}[0]" value="{if isset($value.0)}$value.0{/if}"{if isset($params.width)} style="width:70px"{/if}/><br />
											{l s='Do' mod='prestacenterxmlexportfree'} <input type="text" class="filter datepicker" id="{$params.id_date}_1" name="{$params.name_date}[1]" value="{if isset($value.1)}$value.1{/if}"{if isset($params.width)} style="width:70px"{/if}/>
